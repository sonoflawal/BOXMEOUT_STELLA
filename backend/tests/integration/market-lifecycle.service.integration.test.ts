...
import { PrismaClient, MarketStatus, TradeType, TradeStatus, PredictionStatus } from '@prisma/client';
...
describe('Market Lifecycle Service Integration', () => {
    ...
    afterAll(async () => {
        if (testMarket) {
            ...
            await prisma.prediction.deleteMany({ where: { marketId: testMarket.id } });
    ...
    it('should complete full market lifecycle using services', async () => {
        ...
        const poolResult = await marketService.createPool(testMarket.id, poolLiquidity);
        ...
        const marketAfterPool = await prisma.market.findUnique({ where: { id: testMarket.id } });
        ...
        const userAfterBuy = await prisma.user.findUnique({ where: { id: testUser.id } });
        ...
        const userAfterCommit = await prisma.user.findUnique({ where: { id: testUser.id } });
        expect(Number(userAfterCommit?.usdcBalance)).toBe(Number(userAfterBuy?.usdcBalance) - predictionAmount);
        ...
        const settledPrediction = await prisma.prediction.findUnique({ where: { id: prediction.id } });
        ...
        // 8. Claim Winnings
        const balanceBeforeClaim = (await prisma.user.findUnique({ where: { id: testUser.id } }))?.usdcBalance;

        const claimResult = await predictionService.claimWinnings(testUser.id, prediction.id);
        expect(claimResult.winnings).toBeGreaterThan(0);

        const finalUser = await prisma.user.findUnique({ where: { id: testUser.id } });
        expect(Number(finalUser?.usdcBalance)).toBe(Number(balanceBeforeClaim) + claimResult.winnings);
